...
Since both portfolios have the same payoff at time $T$ by \eqref{eq: P - C}, they have the same price for all times $t$ where $0 \leq t \leq T$ \ref{arbitrage-free pricing sect} (FIXME: fix reference style). 
...
\section{Approximating Arithmetic Asian under the Bachelier Model}

If we assume $r=0$, stock price evolves according to 
\begin{align}
     S_t = S_0 + \kappa W_t
\end{align}
One can price an option sold at time $t=0$ according to
\begin{align}
...
\begin{align}
     V_0 = \rE{(S_T-K)^{+}}
\end{align}
Note that $S_t \sim N(S_0,\kappa^2T)$, as the variance of a Brownian motion is defined as $T$ (FIXME: Cite Brownian and variance math).
...
\end{align}
Plugging this into \eqref{sect 6 t0 price}
\begin{align}
...
The variance of $Z_T$ is $\frac{\kappa^2T}{\sqrt{3}}$, as the variance of a Brownian motion is defined as $T$ (FIXME: Cite Brownian and Variance math) and 
\begin{align}
...
So, we can approximate the price of an Asian in the Bachelier Model by pricing it with the same parameters as a European Option (which can typically be done in O(1) time-complexity),
but just changing the volatility to be $\frac{1}{\sqrt{3}}$ of the original.

This result also applies to Asian Puts through Put-Call Parity (FIXME: Cite), using a European Put with  $\frac{1}{\sqrt{3}}$ of the original volatility.
...
\section{Arbitrage-free pricing} \label{arbitrage-free pricing sect}
